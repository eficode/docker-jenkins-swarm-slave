...
ARGUMENT= -name $SLAVE_NAME -master http://$JENKINS_MASTER_HOST:$JENKINS_MASTER_PORT -labels $JENKINS_SLAVE_LABELS -disableSslVerification -username $JENKINS_MASTER_USERNAME -password $JENKINS_MASTER_PASSWORD -fsroot /var/jenkins_slave_home -mode exclusive
